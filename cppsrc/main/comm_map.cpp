...
int main(int argc, char** argv) {
  ...
  for (int ii = 0; ii < params.pEpisodeSteps; ++ii) {
    ...
    auto comm_maps = env->GetCommunicationMaps(params.pFeatureMapSize);
    for (CoverageControl::MapType &map : comm_maps) {
      auto min_comm = map.minCoeff();
      if (min_comm < 0) {
        std::cout << "Negative communication value: " << min_comm << std::endl;
        break;
      }
    }
    if (ii % 100 == 0) {
